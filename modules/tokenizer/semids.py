...
Converts item feature vectors to discrete semantic IDs using a trained RQ-VAE.
The tokenizer precomputes IDs for all corpus items and caches them for fast
lookup during decoder training.
"""
...
class SemanticIdTokenizer(nn.Module):
    """Tokenizes batches of item features into semantic IDs via a frozen RQ-VAE."""
    ...
    def __init__(
        ...
        rqvae_distance_l2_normalize: bool = False,
    ) -> None:
        super().__init__()
        ...
        self.rq_vae = RqVae(
            ...
            codebook_distance_l2_normalize=rqvae_distance_l2_normalize,
            n_layers=n_layers,
    ...
    @torch.no_grad()
    @eval_mode
    def precompute_corpus_ids(self, item_dataset: ItemEmbeddingDataset) -> Tensor:
        cached_ids = None
        dedup_dim = []
