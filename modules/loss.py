...
class QuantizeLoss(nn.Module):
    ...
    def forward(self, query: Tensor, value: Tensor, ema_update: bool = False) -> Tensor:
        if ema_update:
            # EMA handles codebook updates; only apply commitment loss here.
            return self.commitment_weight * ((query - value.detach()) ** 2).sum(axis=[-1])
