#!/usr/bin/env python3
"""Build and push the amazon-all-beauty-filtered HuggingFace dataset.

Identical to push_to_hf.py, but every interactions row is kept only when
len(history_semantic_ids) >= MIN_HISTORY_LEN (default: 5).

Two configs are created:
  - items        : one row per item (112,590) with metadata, images, semantic IDs
                   (same full catalogue as the unfiltered repo)
  - interactions : train / valid / test splits, filtered to history length >= 5

Usage
-----
  python3 push_to_hf_filtered.py --token <HF_TOKEN>
  python3 push_to_hf_filtered.py --token <HF_TOKEN> --skip_items
  python3 push_to_hf_filtered.py --token <HF_TOKEN> --skip_interactions
  python3 push_to_hf_filtered.py --token <HF_TOKEN> --min_history 5
"""
...
REPO_ID        = "theblackcat102/amazon-all-beauty-filtered"
CATEGORY       = "All_Beauty"
KCORE          = 0
MIN_HISTORY_LEN = 5  # keep rows with at least this many history items
...
# ---------------------------------------------------------------------------
# Step 2: Build interactions dataset (filtered)
# ---------------------------------------------------------------------------
...
def build_interactions(
    ...
    Only rows with len(history_semantic_ids) >= min_history are kept.
    """
    print(f"\nBuilding interactions dataset (min_history={min_history}) ...")
    ...
    for split_name, jsonl_path in splits_jsonl.items():
        ...
        print(f"  {split_name}: {len(query_rows):,} query rows (before filter)")
        ...
        for row in tqdm(query_rows, desc=f"  Processing {split_name}", leave=False):
            ...
            # --- FILTER: require at least min_history history items ---
            if len(history_semids) < min_history:
                skipped_short += 1
                continue
        ...
        print(
            f"    Kept {len(records):,} rows | "
            f"Skipped — no_seq: {skipped_no_seq:,}  "
            f"target_missing: {skipped_no_target:,}  "
            f"history_too_short: {skipped_short:,}"
        )
...
# ---------------------------------------------------------------------------
# Step 3: Build items dataset (full catalogue, no history filter)
# ---------------------------------------------------------------------------
...
def main():
    parser = argparse.ArgumentParser(
        description="Push amazon-all-beauty-filtered dataset to HuggingFace."
    )
    ...
    # --- Step 2: Interactions config (filtered) ---
    if not args.skip_interactions:
        interactions = build_interactions(
            user_sequences, semid_tokens, semid_codes,
            min_history=args.min_history,
        )
        print(f"\nInteractions DatasetDict:")
        print(interactions)
        print("\nSample train row:")
        if len(interactions["train"]) > 0:
            sample = interactions["train"][0]
            print({k: v if k != "target_text" else v[:80] + "..." for k, v in sample.items()})
