#!/usr/bin/env python3
"""3×3 Evaluation Grid for Generative Recommendation.
...
Dataset splits follow the leave-one-out protocol:
  - Test  : absolute last item as target, all preceding items as input.
  - Valid : second-to-last item as target (used only during training
            of the text-modality model to monitor performance).
...
For each cell (train_mod, test_mod):
  ...
  3. Re-tokenize the test sequences using test_mod embeddings through
...
def evaluate_cell(
    ...
    with tqdm(test_dataloader, desc="    Evaluating", leave=False) as pbar:
        for batch in pbar:
...
def print_grid_table(grid_results: dict, category: str, modalities: list = None) -> None:
    ...
    print(f"\n{'='*80}")
    print(f"  3x3 Evaluation Grid -- Category: {category}")
    print(f"{'='*80}")
...
def main():
    parser = argparse.ArgumentParser(description="3×3 evaluation grid.")
    parser.add_argument(
        "--category",
        type=str,
        default="All_Beauty",
    ...
    print(f"\nRunning 3×3 grid for category: {args.category}")
    results = run_grid(
    ...
    # Save results
    out_path = output_dir / f"{args.category}_grid_results.json"
